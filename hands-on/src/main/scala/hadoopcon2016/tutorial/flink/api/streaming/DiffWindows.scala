...
object DiffWindows {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //   2. set the time characteristic as event time
    //   3. prepare the data stream by means of fromCollection()
    //   4. transform the data set into the desired format or results
    //   5. extract timestamp from each event
    //   6. specify where to save results of the computation
    //   7. trigger the program execution
  }
}
