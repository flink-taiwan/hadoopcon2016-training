...
object WindowFuncs {
  ...
  def main(args: Array[String]): Unit = {
    // Hints:
    //    1. obtain a StreamExecutionEnvironment
    //    2. specify the time characteristic as event time
    //    3. prepare the data stream by means of fromCollection()
    //    4. transform the data stream into desired format or results
    //    5. specify how to extract the timestamp for each event
    //    6. specify which window assigner will be used to generate windows
    //    7. specify window functions to process events in a window
    //    8. specify where to save results of the computation
    //    9. trigger the program execution
  }
}
